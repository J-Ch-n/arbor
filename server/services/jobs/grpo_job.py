...
class GRPOJob(Job):
    ...
    def _build_trainer_config(
        self, request: GRPOInitializeRequest, output_dir: str, vllm_port: int, vllm_group_port: int
    ) -> ArborGRPOConfig:
        ...
        config.vllm_server_group_port = vllm_group_port
    ...
    def initialize(
        ...
        self.trainer_config: ArborGRPOConfig = self._build_trainer_config(
            request, checkpoint_dir, self.inference_job.port, self.inference_job.group_port
        )
